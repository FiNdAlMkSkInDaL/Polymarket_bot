#!/usr/bin/env python3
"""Convenience entrypoint for OFI momentum WFO on the March 2026 Top 25 universe."""
...
DEFAULT_REPORT_DIR = Path("data/wfo_ofi_momentum_top25")
...
DEFAULT_BOUNDS_PATH = Path("data/wfo_ofi_momentum_bounds.json")
TARGET_PARAMS = (
    "ofi_threshold",
    "window_ms",
    "ofi_tvi_kappa",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OFI momentum WFO on the March 2026 Top 25 market universe."
    )
    ...
    parser.add_argument(
        "--market-configs",
        default=str(DEFAULT_MARKET_CONFIGS),
        help="Path to the Top 25 market map JSON with yes/no asset IDs.",
    )
    ...
    parser.add_argument(
        "--search-space-bounds",
        default=str(DEFAULT_BOUNDS_PATH),
        help="Optional JSON file that narrows the shared WFO search bounds for OFI momentum.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = WfoConfig(
        ...
        search_space_bounds_path=args.search_space_bounds,
        strategy_adapter="bot_replay",

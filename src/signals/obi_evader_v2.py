...
class ObiEvaderV2(BaseStrategy):
    ...
    min_quote_spread_cents: Decimal = Decimal("2.0")
    ...
    def __post_init__(self) -> None:
        ...
        self.min_quote_spread_cents = self._as_decimal(self.min_quote_spread_cents, name="min_quote_spread_cents")
        ...
        if self.min_quote_spread_cents <= Decimal("0"):
            raise ValueError("min_quote_spread_cents must be strictly positive")
    ...
    def on_bbo_update(self, market_id: str, top_bids: Sequence[dict[str, Any]], top_asks: Sequence[dict[str, Any]]) -> None:
        ...
        spread_cents = (asks[0].price - bids[0].price) * Decimal("100")
        if spread_cents <= self.min_quote_spread_cents:
            self._active_mode[normalized_market_id] = "GATED"
            return

        if abs_obi < self.safe_zone_abs_obi:
            self._quote_market(normalized_market_id, bids[0].price, asks[0].price, obi, spread_cents)
        ...
        self.submit_order(
            PriorityOrderContext(
                ...
                signal_metadata={
                    ...
                    "quote_side": "BID",
                    "quote_id": f"obi_evader_v2:{market_id}:bid",
                    "spread_cents": str(spread_cents),
                    "min_quote_spread_cents": str(self.min_quote_spread_cents),
                },
            )
        )
        self.submit_order(
            PriorityOrderContext(
                ...
                signal_metadata={
                    ...
                    "liquidity_intent": "MAKER",
                    "quote_side": "ASK",
                    "quote_id": f"obi_evader_v2:{market_id}:ask",
                    "spread_cents": str(spread_cents),
                    "min_quote_spread_cents": str(self.min_quote_spread_cents),
                },
            )

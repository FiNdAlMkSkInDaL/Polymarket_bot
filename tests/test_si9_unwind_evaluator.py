...
def test_cluster_id_must_match_original_manifest() -> None:
    evaluator = Si9UnwindEvaluator(_config())
...
def test_unknown_hanging_leg_market_id_raises_value_error() -> None:
    evaluator = Si9UnwindEvaluator(_config())

    with pytest.raises(ValueError, match="Unknown hanging leg"):
        evaluator.evaluate(
            cluster_id="cluster-1",
            hanging_legs=[("mkt-z", Decimal("1"), Decimal("0.40"))],
            current_bids={"mkt-z": Decimal("0.38")},
            unwind_reason="MANUAL_ABORT",
            original_manifest=_manifest(),
            unwind_timestamp_ms=200,
        )
...
def test_missing_current_best_bid_raises_value_error() -> None:
    evaluator = Si9UnwindEvaluator(_config())

    with pytest.raises(ValueError, match="Missing current best bid"):
        evaluator.evaluate(
            cluster_id="cluster-1",
            hanging_legs=[("mkt-a", Decimal("1"), Decimal("0.40"))],
            current_bids={},
...
def test_zero_filled_size_raises_value_error() -> None:
    evaluator = Si9UnwindEvaluator(_config())

    with pytest.raises(ValueError, match="filled_size"):
        evaluator.evaluate(
            cluster_id="cluster-1",
            hanging_legs=[("mkt-a", Decimal("0"), Decimal("0.40"))],
            current_bids={"mkt-a": Decimal("0.38")},
            unwind_reason="MANUAL_ABORT",
            original_manifest=_manifest(),
            unwind_timestamp_ms=200,
...
@pytest.mark.parametrize("filled_price", [Decimal("0"), Decimal("1")])
def test_invalid_filled_price_bounds_raise_value_error(filled_price: Decimal) -> None:
    evaluator = Si9UnwindEvaluator(_config())

    with pytest.raises(ValueError, match="filled_price"):
        evaluator.evaluate(
            cluster_id="cluster-1",
            hanging_legs=[("mkt-a", Decimal("1"), filled_price)],
            current_bids={"mkt-a": Decimal("0.38")},
            unwind_reason="MANUAL_ABORT",
            original_manifest=_manifest(),
            unwind_timestamp_ms=200,
...
def test_unwind_legs_are_sorted_by_manifest_leg_index() -> None:
    evaluator = Si9UnwindEvaluator(_config())

    manifest = evaluator.evaluate(
        cluster_id="cluster-1",
        hanging_legs=[
            ("mkt-c", Decimal("1"), Decimal("0.40")),
            ("mkt-a", Decimal("1"), Decimal("0.40")),
        ],
        current_bids={"mkt-a": Decimal("0.38"), "mkt-c": Decimal("0.39")},
        unwind_reason="MANUAL_ABORT",
        original_manifest=_manifest(),
        unwind_timestamp_ms=200,
    )

    assert [leg.market_id for leg in manifest.hanging_legs] == ["mkt-a", "mkt-c"]


def test_escalation_updates_unwind_timestamp_on_new_manifest() -> None:
    evaluator = Si9UnwindEvaluator(_config(max_hold_recovery_ms=100))
    manifest = evaluator.evaluate(
        cluster_id="cluster-1",
        hanging_legs=[("mkt-a", Decimal("1"), Decimal("0.40"))],
    ...
    assert escalated.unwind_timestamp_ms == 301
    assert manifest.unwind_timestamp_ms == 200


def test_evaluate_preserves_original_manifest_reference() -> None:
    evaluator = Si9UnwindEvaluator(_config())
    original_manifest = _manifest()
    ...
    assert manifest.original_manifest is original_manifest


def test_unsupported_unwind_reason_raises_value_error() -> None:
    evaluator = Si9UnwindEvaluator(_config())

    with pytest.raises(ValueError, match="unsupported unwind_reason"):
        evaluator.evaluate(
            cluster_id="cluster-1",
            hanging_legs=[("mkt-a", Decimal("1"), Decimal("0.40"))],
            current_bids={"mkt-a": Decimal("0.38")},
            unwind_reason="UNKNOWN_REASON",
            original_manifest=_manifest(),
            unwind_timestamp_ms=200,
        )


def test_market_sell_reason_has_priority_over_passive_cost_band() -> None:
    evaluator = Si9UnwindEvaluator(_config(market_sell_threshold=Decimal("1.0"), passive_unwind_threshold=Decimal("0.5")))

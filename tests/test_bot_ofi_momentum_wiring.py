...
async def test_l2_bbo_change_routes_ofi_signal_through_live_bot() -> None:
    ...
    bot._ofi_detectors[market.condition_id] = OFIMomentumDetector(
    ...
    bot._on_panic_signal.assert_awaited_once()
    call = bot._on_panic_signal.await_args
    assert call.args[0].signal_source == "ofi_momentum"
    assert call.args[0].direction == "BUY"
    assert call.args[0].toxicity_index == pytest.approx(0.82, abs=1e-6)
    assert call.kwargs["signal_metadata"]["signal_source"] == "ofi_momentum"
    assert call.kwargs["signal_metadata"]["toxicity_index"] == pytest.approx(0.82, abs=1e-6)
